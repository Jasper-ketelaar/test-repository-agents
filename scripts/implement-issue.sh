...
BODY
  ...
  local pr_json_file pr_err_file
  pr_json_file=$(mktemp)
  pr_err_file=$(mktemp)
  ...
  if ! gh api \
    ...
    -f head="$branch_name" \
    -f base="$BASE_BRANCH" \
    -f body="$pr_body" >"$pr_json_file" 2>"$pr_err_file"; then
    local pr_err
    pr_err=$(tr '\n' ' ' <"$pr_err_file")
    rm -f "$pr_json_file" "$pr_err_file"
    cleanup_on_error "pull request create failed: ${pr_err}"
  fi

  pr_url=$(jq -r '.html_url // empty' "$pr_json_file")
  pr_number=$(jq -r '.number // empty' "$pr_json_file")
  rm -f "$pr_json_file" "$pr_err_file"

  if [[ -z "$pr_number" || -z "$pr_url" ]]; then
    cleanup_on_error "Failed to parse PR number/URL from create-pull response"
  fi
